...
class ImageHelper {
	public static boolean save_image(BufferedImage img, String path) {
		try {
			...
			System.out.println(" >> Der angegebene Pfad hat keine gültige Bilddatei");
			return false;
		}
		return true;
	}
...
public class k02_steganographie {
	...
	private static BufferedImage get_image(Scanner sc) {
		...
		String path = "";
		if (first_run < 2) {
			System.out.println("(Sie können eine leere Zeile bestätigen für das Default Bild)");
			first_run += 1;
		}
		while (img == null) {
			...
			if (input.length() > 0) {
				if (!input.contains(".png")) {
					System.out.println(" >> Der Pfad muss zu einem gültigen PNG-Bild führen!");
					continue;
				}
				path = input;
			} else {
	...
	private static void read_message(Scanner sc) {
		...
		if (contains_hidden_message) {
			...
			System.out.println("Das Bild enthält keine geheime Nachricht (mit diesem Programm erzeugt)\n");
		}
	...
	private static void hide_message(Scanner sc) {
		...
		if (success) {
			if (wrote_full_message) {
				...
				System.out.println("\nDas Bild enthält nicht genug Platz für die gesamte Nachricht."
